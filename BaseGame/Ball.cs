...
namespace BaseGame
{
    class Ball : PictureBox
    {
        ...
        public bool collision(PictureBox obj)
        {
            ...
            {
                /* In this case, if the left side of the ball collides while the ball is moving left, flip its direction
                 * else, move it at maximum horizontal speed in the same direction (simulating a corner collision) */
                ...
                bottomCenter.Y <= obj.Location.Y + obj.Height)
            {
                ...
                // If the ball hits the player I will add an additional mechanic
                if (obj is Player)
                {
                    float surfaceHit = 0;
